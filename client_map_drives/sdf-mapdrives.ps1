...
if (!$log_path_exists){
    ...
    echo "Log path absent. Directory $log_path created." | out-file $log_file -Append
}


# Rotate the log if needed
if (((Get-ChildItem $log_file).Length/1KB) -ge $log_rotate_threshold_in_kb){
    echo "File bigger than $log_rotate_threshold_in_kb KB - Rotating log" | out-file $log_file -Append
    $datetime = Get-Date -uformat "%Y-%m-%d-%H%M"
    $newname = "$((Get-ChildItem $log_file).BaseName)-${datetime}.log_old"
    Rename-Item $log_file $newname
    echo "Rotated file name is $log_path\$newname" | out-file $log_file -Append
}

# Yay we're starting!  (it's for the log)
echo "=== Script begin $(get-date) ===" | out-file $log_file -Append
...
if ($SDrive_Exist){
    ...
    echo "S Drive Exists" | out-file $log_file -Append
}
elseif ($SDrive_Path_Exist) {
    # If it does not exist, check if the path exists and then map it
    echo "S Drive absent. Mapping..." | out-file $log_file -Append
...
if ($LDrive_Exist){
    # If it exists, move to the next command
    echo "L Drive Exists" | out-file $log_file -Append
}
elseif ($LDrive_Path_Exist){
    # If it does not exist, check if the path exists and then map it
    echo "L Drive absent. Mapping..." | out-file $log_file -Append
...
# Finito. (for the log)
echo "=== Script end $(get-date) ===" | out-file $log_file -Append
echo "*******************************" | out-file $log_file -Append
